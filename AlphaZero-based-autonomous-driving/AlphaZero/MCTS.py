...
class MCTSNode:
    def __init__(self, env, parent, parent_action, prior_prob):
        ...
        if self.parent is None:
            ...
            self.stack_of_planes = get_stack_of_grid(self.parent.stack_of_planes, observation)

    def pucb_score(self, c_puct=2):
        """Compute PUCB score for current node."""
    ...
    def select(self, c_puct=3):
        """Select child node with the highest PUCB score."""
        if not self.children:
            return None
        return max(self.children.values(), key=lambda child: child.pucb_score(c_puct))

    def expand(self, action_priors):
        """Expand current node with child nodes from action priors."""
    ...
    def is_leaf(self):
        """Check whether current node has no children."""
        return self.children == {}

    def backpropagate(self, result):
        """Update visit count and value sum at this node."""
        self._n += 1
        self._W += result

    def backpropagate_recursive(self, result):
        """Backpropagate from current node up to root."""
        if self.parent:
...
class MCTS:
    ...
    def move_to_new_root(self, action):
        """Move tree root to the selected action child node."""
