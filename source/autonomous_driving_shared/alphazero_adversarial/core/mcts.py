        ...
        if self.env is not None:
            ...
            if self.history is None:
                if self.parent is None:
                    self.history = seed_history_from_env(
                        self.env,
                    ...
                    if self.parent.history is None:
                        raise RuntimeError(
                            "Parent history must exist before child history can be derived."
                        )
...
class BaseSimultaneousMCTS:
    ...
    def _prepare_network_inputs(
        ...
        return batch, target_vector_batch

    def _forward_network(
        self,
        node: SimultaneousMCTSNode,
    ):
        batch, target_vector_batch = self._prepare_network_inputs(node)
        if self._device.type == "cuda":
            torch.cuda.synchronize(self._device)
        started_at = time.perf_counter()
        with torch.inference_mode():
            outputs = self._network(batch, target_vector_batch)
        if self._device.type == "cuda":
            torch.cuda.synchronize(self._device)
    ...
    @staticmethod
    def _clip_value_batch(value_batch) -> tuple[float, float]:
