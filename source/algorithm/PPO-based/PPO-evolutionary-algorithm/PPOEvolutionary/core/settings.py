...
def _get_action_count(
    *,
    ...
    raise ValueError(f"Unsupported action type {action_type!r} for PPO-evolutionary settings.")
...
@dataclass(frozen=True)
class LoggingConfig:
    metrics_path: str = "outputs/ppo_evolutionary_metrics.jsonl"
...
class PPOEvolutionaryConfig:
    ...
    model_path: str = "models/ppo_evolutionary_highway.pt"
    ...
    def __post_init__(self) -> None:
        ...
        if int(self.rollout.episodes_per_policy) <= 0:
            raise ValueError("rollout.episodes_per_policy must be positive.")
        if int(self.rollout.workers) <= 0:
            raise ValueError("rollout.workers must be positive.")
        ...
        if float(self.evolution.elite_fraction) <= 0.0:
    ...
    def from_dict(
        ...
        *,
        ...
        data["evolution"] = EvolutionConfig.from_dict(data.pop("evolution", {}))
...
def load_stage_config(
    ...
    *,
    ...
    return PPOEvolutionaryConfig.from_dict(
        preset,
...
def reload_settings() -> tuple[PPOEvolutionaryConfig, PPOEvolutionaryConfig]:
