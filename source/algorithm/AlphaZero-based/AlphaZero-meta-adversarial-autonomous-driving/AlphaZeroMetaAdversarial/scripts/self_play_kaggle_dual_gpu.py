...
DEFAULT_OUTPUT_DIR = Path("/kaggle/working/alphazero_meta_adversarial_self_play")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run meta-adversarial AlphaZero self-play on Kaggle with one worker per GPU."
    )
...
def main() -> int:
    ...
    print(
        "[meta-adversarial-self-play-kaggle-dual-gpu] "
    ...
    for worker_id, (device, episode_count) in enumerate(
        ...
        process = ctx.Process(
            ...
            name=f"meta-adversarial-self-play-worker-{worker_id}",
        )
        process.start()
