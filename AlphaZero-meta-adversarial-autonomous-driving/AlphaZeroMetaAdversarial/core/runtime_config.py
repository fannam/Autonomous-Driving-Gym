...
def resolve_config_path(config_path: str | os.PathLike[str] | None = None) -> Path:
    raw_path = config_path or os.environ.get("ALPHAZERO_META_ADVERSARIAL_CONFIG_PATH")
...
def _load_serialized_config(path: Path) -> dict[str, Any]:
    ...
    if yaml is not None:
        ...
        try:
            ...
            raise RuntimeError(
                "PyYAML is not installed, so the meta-adversarial config must use JSON-compatible YAML syntax."
            ) from exc
...
def get_active_scenario_name(
    ...
    *,
    ...
    override_name = os.environ.get("ALPHAZERO_META_ADVERSARIAL_SCENARIO")
    ...
    if override_name and override_name != scenario_name:
        ...
        raise ValueError(
            f"ALPHAZERO_META_ADVERSARIAL_SCENARIO={override_name!r} does not match "
