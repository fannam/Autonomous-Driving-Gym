...
OTHER_VEHICLES="${OTHER_VEHICLES:-1}"
...
if [[ -n "$SOURCE_MODEL" ]]; then
  ...
  source_model_display="bootstrap:model_iter_0000.pth"
fi

printf '[self-play] iteration=%s source_model=%s workers=%s %s device=%s n_simulations=%s result_timeout=%s\n' \
  "$ITERATION" "$source_model_display" "$WORKERS" "$episode_mode" "$DEVICE" "$N_SIMULATIONS" "${RESULT_TIMEOUT:-disabled}"
