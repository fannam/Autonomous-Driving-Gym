...
OUTPUT_DIR="${OUTPUT_DIR:-/kaggle/working/ppo_traditional_highway}"
GENERATED_CONFIG_PATH="${GENERATED_CONFIG_PATH:-$OUTPUT_DIR/highway_ppo_traditional_kaggle.yaml}"
CHECKPOINT_PATH="${CHECKPOINT_PATH:-$OUTPUT_DIR/ppo_traditional_highway.pt}"
...
printf '[run-ppo-traditional-train-kaggle] scenario=%s base_config=%s generated_config=%s output_dir=%s updates=%s n_envs=%s device=%s checkpoint=%s metrics=%s note=%s\n' \
