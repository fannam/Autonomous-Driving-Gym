...
EXPECTED_EVAL_RELATIVE="source/algorithm/PPO-based/PPO-evolutionary-algorithm/PPOEvolutionary/scripts/evaluate.py"
...
resolve_checkpoint_path() {
  ...
  local candidates=(
    "$REPO_ROOT/outputs/ppo_evolutionary_highway/ppo_evolutionary_highway.pt"
    "$REPO_ROOT/outputs/ppo_evolutionary_highway_local/ppo_evolutionary_highway.pt"
    "$REPO_ROOT/ppo_evolutionary_highway.pt"
    "$PWD/ppo_evolutionary_highway.pt"
  )
  ...
  local discovered
  discovered="$(find "$REPO_ROOT" -maxdepth 5 -type f -name 'ppo_evolutionary_highway.pt' | head -n 1 || true)"
  if [[ -n "$discovered" ]]; then
    printf '%s\n' "$discovered"
    return 0
  fi

  echo "Could not find ppo_evolutionary_highway.pt. Set CHECKPOINT_PATH=/abs/path/to/ppo_evolutionary_highway.pt" >&2
  exit 1
}
...
OUTPUT_DIR="${OUTPUT_DIR:-$REPO_ROOT/outputs/ppo_evolutionary_infer_local}"
GENERATED_CONFIG_PATH="${GENERATED_CONFIG_PATH:-$OUTPUT_DIR/highway_ppo_evolutionary_eval_from_checkpoint.json}"
...
printf '[run-ppo-evolutionary-infer-local] repo_root=%s checkpoint=%s config=%s policy_source=%s episodes=%s seed_start=%s render_mode=%s device=%s\n' \
  "$REPO_ROOT" \
