...
printf '[self-play-highway-gpu] scenario=%s config=%s iteration=%s source_model=%s workers=%s %s device=%s n_simulations=%s num_gpus=%s gpu_indices=%s result_timeout=%s\n' \
  "$ALPHAZERO_SCENARIO" \
