...
DEVICE="${DEVICE:-cpu}"
N_SIMULATIONS="${N_SIMULATIONS:-200}"
...
printf '[self-play-highway] scenario=%s config=%s iteration=%s source_model=%s workers=%s %s device=%s n_simulations=%s result_timeout=%s\n' \
  "$ALPHAZERO_SCENARIO" \
