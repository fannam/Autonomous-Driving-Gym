...
from PPOEvolutionary.core.runtime_config import get_scenario_config_path
from PPOEvolutionary.core.settings import EVALUATION_CONFIG, TRAIN_CONFIG
from PPOEvolutionary.environment.config import build_env_spec
from tools.repo_layout import PPO_EVOLUTIONARY_ROOT


def test_ppo_evolutionary_runtime_config_finds_package_layout() -> None:
    expected = (
        PPO_EVOLUTIONARY_ROOT / "configs" / "highway_ppo_evolutionary.yaml"
    ).resolve()
    config_path = get_scenario_config_path("highway_ppo_evolutionary")
...
def test_ppo_evolutionary_stage_env_overrides_success_termination() -> None:
    config_path = (
        PPO_EVOLUTIONARY_ROOT / "configs" / "highway_ppo_evolutionary.yaml"
    ).resolve()
